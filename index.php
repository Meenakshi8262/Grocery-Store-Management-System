...
// Task 1: Load Grocery Items from Session
function loadInventory() {
    // Check if inventory exists in session
    if (isset($_SESSION['groceryItemsDetails'])) {
        return $_SESSION['groceryItemsDetails'];
    } else {
        // Default inventory if session doesn't exist
        return [
...
// Task 2: Save Grocery Items to Session
function saveInventory($inventory) {
    $_SESSION['groceryItemsDetails'] = $inventory;
}
...
function addItem($name, $type, $price, $expiry) {
    ...
    // Save the updated inventory back to the session
